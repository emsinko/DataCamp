...
credit_model <- bagging(formula = default ~ ., 
                        data = credit_train,
                        coob = TRUE) # coob je na to, že tie data, ktore sa nepoužili v bootstrap sample, su použité ako testovacia sada
...
#výhody: Vie vypoèita performance aj bez delenia na test a train set pomocou OOB (out of the bag)
#nevýhody : Cez OOB nevieme zohna AUC , nevieme RF porovna s inými modelmi
...
## Defaultne je nastavených 500 stromov v RF 
## Poèet vybranych premennych je defaultne mtry=sqrt(#premennych)
...
# v prípade i stromov. Tým pádom fby hodnota v ståpci OOB cca klesa
# a ak si to dáme plotnut tento objekt, zistíme ko¾ko stromov nám treba
# v princípe sa tento error rate ustáli a v rámci šetrenia èasu výpoètu môžeme troška obmedzi poèet stromov v lese
